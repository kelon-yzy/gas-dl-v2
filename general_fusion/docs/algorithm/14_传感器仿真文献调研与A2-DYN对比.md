...
> 版本说明：本文记录 R4 实施前的评审快照；其中“当前计划”“2 Hz 首选”和“尚未实现”等表述不覆盖后续执行事实。R4 已在文档 13 对应代码中完成 direct HEOS、设备链和 pilot 资格审计，并冻结 `5 Hz / 240 s` 与 `US-CHIRP-XCORR-PARABOLIC-1`；完整 6,300 观测数据包已生成，但 R2 冻结重审计因 test NOISE-10X 动态非退化失败而终止于 `DATA_FREEZE_FAILED`。
