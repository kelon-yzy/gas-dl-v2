...
与 hydrogen_ng `sim.generation.slow` 并存。差异：
- SLOW_CHANNELS / SLOW_DYNAMIC_CHANNELS 来自 syngas_schema（含 V_NDIR_CO，9/4 通道）
- 物理函数走 sim.generation.syngas.acoustic_physics
- 波形仿真通过 sound_speed_fn / attenuation_fn 注入 syngas 物理
- HITRAN 后端通过 sim.generation.syngas.optical_backend 走 3 通道（ch4/co2/co）光谱前向，
  CO 通道与 hg 完全隔离（独立网格 [1980, 2310] cm⁻¹）。
"""
...
from sg.sim.core.syngas_schema import SLOW_CHANNELS, SLOW_DYNAMIC_CHANNELS
from sg.sim.generation.optical_backend import (
    EMPIRICAL_ABSORPTION_BACKEND,
    HITRAN_ABSORPTION_BACKEND,
    VALID_OPTICAL_ABSORPTION_BACKENDS,
)
from sg.sim.generation.phases import PhaseSchedule, resolve_phase_schedule
from sg.sim.generation.spectral import HitranGridSpec, PreparedTabulatedSpectra
...
from sg.sim.generation.syngas.optical_backend import compute_hitran_syngas_optical_absorption
from sg.sim.generation.waveforms import (
    FiberMicSpec,
    WaveformSpec,
...
# 系统响应时间常数（与 hydrogen_ng 同量级，CO 通道按 CO2 量级类比）
TAU_RISE_SYSTEM_S = {
    "V_NDIR_CH4": (8.0, 20.0),
    "V_NDIR_CO2": (6.0, 18.0),
    "V_NDIR_CO": (6.0, 18.0),
...
def build_sequence_arrays(
    ...
    *,
    ...
    for seq_index, condition in enumerate(conditions):
        ...
        # baseline = 100% N2 等价（其他组分为 0）
        baseline_condition = _main_feature_condition(
            condition, x_h2=0.0, x_ch4=0.0, x_co2=0.0, x_co=0.0, x_n2=100.0,
        ...
        for timestep in range(timesteps):
            ...
            if is_empirical:
                ...
                ndir_equilibrium = _hitran_ndir_equilibrium_syngas(
                    condition,
                ...
                current.update(
                    _dynamic_features_from_equilibrium(
                        ...
                        slow_walk,
                        sequence_rng,
                        channels=("V_NDIR_CH4", "V_NDIR_CO2", "V_NDIR_CO"),
                    )
                )
            ...
            # 波形仿真：注入 syngas 物理。extra_gas_kwargs 透传 x_co；
            # composition dict 已含 x_h2/x_ch4/x_co2/x_n2/x_co。
            extra_gas = {"x_co": composition["x_co"]}
...
def _hitran_ndir_equilibrium_syngas(
    condition: dict[str, str],
    *,
    ...
    """对当前 blended composition 计算 3 通道 NDIR 平衡电压。

    走 HITRAN 后端的 ``compute_hitran_syngas_optical_absorption`` 拿到 absorbance，
    转回 V_NDIR_{CH4,CO2,CO} = baseline * exp(-A_observed)。floor 0.1 V 防止
    极端工况下电压塌到 0。
    """
    optical_condition = _main_feature_condition(
        condition,
        x_h2=composition["x_h2"],
        x_ch4=composition["x_ch4"],
        x_co2=composition["x_co2"],
        x_co=composition["x_co"],
        x_n2=composition["x_n2"],
        l_m=l_m,
    )
    optical = compute_hitran_syngas_optical_absorption(
        optical_condition,
    ...
    return {
        "V_NDIR_CH4": max(
            0.1,
            PROCESSING_PARAMS["optical_baseline_ch4_init"]
            * math.exp(-float(optical["absorption_ch4_observed"])),
        ),
        "V_NDIR_CO2": max(
            0.1,
        ...
        "V_NDIR_CO": max(
            0.1,
            PROCESSING_PARAMS["optical_baseline_co_init"]
            * math.exp(-float(optical["absorption_co_observed"])),
        ),
    }


def _blend_composition(condition: dict[str, str], blend: float) -> dict[str, float]:
    """从纯 N2 baseline 到 target 的线性混合。

    baseline = 100% N2，其他组分为 0；target = condition 给出的实际组分。
    ...
    return {
        ...
        "x_co": float(condition["x_CO"]) * blend,
...
def _slow_row(sequence_id: str, timestep: int, dt_s: float, phase_id: str, current: dict[str, float]) -> dict[str, str]:
    return {
        ...
        "V_NDIR_CO": _fmt(float(current["V_NDIR_CO"]), 6),
