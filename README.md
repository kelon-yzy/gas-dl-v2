...
- 已建立 `src/sim/generation`：提供正式 benchmark 生成的最小垂直切片，默认使用拉丁超立方采样（LHS），NDIR 默认走 `hitran_hapi_v1` cache-only 光谱积分；`empirical_v1` 保留为显式兼容/对照路径。
...
- 已建立测试入口：`python -m pytest tests`（125 个测试，覆盖 sim + dl + pipeline）。
