...
class TestSyngasUnaffected:
    def test_syngas_benchmark_still_loads_with_syngas_scheme(self, tmp_path: Path):
        ...
        dataset_dir = tmp_path / "sg-dl-smoke"
        output_dir = tmp_path / "runs" / "sg-baseline"
        parser = build_dl_cli_parser()
        args = parser.parse_args(
            [
                "--dataset-dir",
        ...
        payload = run_dl_cli(args)
        # sg：9 通道（含 V_NDIR_CO）、4 列目标、按 co_bins/ch4_bins 分箱、sum_abs_error 为 None
