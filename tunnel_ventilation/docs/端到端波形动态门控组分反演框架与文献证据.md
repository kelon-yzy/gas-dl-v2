...
- 当前问题已从“绝对峰位坐标丢失”收敛为“单一峰位坐标加 learned embedding 与固定 `last/mean/max` 聚合不具备 B1 信息充分性”。E1d 正式诊断（`outputs/tv3_ec_msw/e1d_s20260704/`）已完成：正对照通过；TOF-L 校准组 alone 未补回 O₂；在校准栈上加入 SNR 后 compact 集合 `cal_plus_corr_psr_snr` 过门。E1d-SB 正式（`e1d_sb_s20260704/`）已 `parity_passed`。下一步是 E1r attachment 联合审计；E2 FiLM/attention 仍禁止。
