...
- 当前问题已从“绝对峰位坐标丢失”收敛为“单一峰位坐标加 learned embedding 与固定 `last/mean/max` 聚合不具备 B1 信息充分性”。E1d 冻结表示诊断管线已落地（`scripts/run_ec_msw_e1d.py`），逐组检查 phase/时序聚合、delay/TOF-L 校准和 waveform quality；clean 6000 正式诊断待服务器完整 RawDSP cache 执行；E2 仍禁止启动。
