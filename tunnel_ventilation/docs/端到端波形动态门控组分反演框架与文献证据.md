...
- 工程验证已通过：E1 与独立审计共 11 项测试、相关 D2 与波形归一化 58 项回归测试全部通过；`tv3-smoke7` 的 1 epoch smoke 已生成 checkpoint、run config 和 val/test 指标。
- 独立审计链已实现：冻结 encoder 后分别使用 train-only Ridge probe 评价 frame peak fidelity 与 B1 parity，并输出固定 0.8 vol% O₂ 窗口指标和 SHA-256 provenance。本地 smoke 审计按正式门得到 `frame_fidelity_failed`、`e2_allowed=false`，只证明失败可被清晰暴露。
- 尚未完成 clean `tv3-formal-6000` 的 frame fidelity 与 B1 parity，因此当前只有链路可运行证据，不能声明组分性能改善，也不能启动 E2。
