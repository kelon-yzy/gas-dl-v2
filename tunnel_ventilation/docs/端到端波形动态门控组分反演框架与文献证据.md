...
- E1r 前端修复已实现：复用 RawDSP train-only baseline median `template.npy` 做冻结匹配滤波，将归一化绝对峰位作为 frame embedding 第一维并绕过 learned projection；不读取 peak/TOF 真值。`tv3-smoke7` 的 frame fidelity 已通过，val/test/extrapolation 的 MAE 为 `0.00526 / 0.00573 / 0.00455 sample`，P95 为 `0.01045 / 0.00819 / 0.01042 sample`。该 smoke 只证明前端修复有效，clean 6000 preflight/parity 未完成，E2 仍禁止启动。
...
失败 E1 在 `stride=2`、`avg_pool factor=4` 后仅保留分支激活的 mean/max/centroid，再用 `last/mean/max` 聚合跨帧信息。训练只接受组分损失，没有不变量保证学习后激活仍保留原始峰坐标。E1r 已在下采样前补入可部署位置坐标；当前下一门是 clean 6000 preflight 与正式 B1 parity，不得通过放宽门限或直接进入 E2 绕过验收。
