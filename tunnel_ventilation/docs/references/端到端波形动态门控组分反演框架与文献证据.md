...
> 项目：掘进通风三组分检测（tv3）  
> 检索日期：2026-07-13  
> 进展回填：2026-07-17（E1d / E1d-SB / attachment / LS / D1 deploy probe）  
> 检索来源：CrossRef、arXiv（T1），Semantic Scholar（T2）  
> 适用范围：原始超声波形、环境参数、时间窗动态融合与 CO₂/O₂/N₂ 组分回归

## 实施状态与进展分析（截至 2026-07-17）

### 总览

| 阶段                              | 正式 verdict                     | 对原框架主张的含义                                     |
| ------------------------------- | ------------------------------ | --------------------------------------------- |
| E0                              | 已冻结 B1/B7                      | 审计锚点成立                                        |
| E1                              | `frame_fidelity_failed`        | learned encoder 丢失峰位 / TOF；触发 E1r             |
| E1r                             | frame 过门 / `b1_parity_failed`  | 坐标可恢复，但固定 `last/mean/max` + embedding 信息不足    |
| E1d                             | `minimal_deployable_set_found` | compact RawDSP 表示可恢复 B1 非劣                    |
| E1d-SB                          | `parity_passed`                | 可部署 builder `cal_plus_corr_psr_snr` 正式过门      |
| attachment                      | `attachment_passed`            | E1r 帧保真 + e1d_sb 序列联合成立                       |
| E2s-LS                          | `ls_ablation_passed`，**不晋升**   | 相对 e1d_sb 的 O₂ ΔR² 仅约 `+0.0005～0.001`         |
| D1 deploy probe                 | `deploy_probe_passed`          | waveform→e1d_sb→Ridge→raw3 接线成立；与 e1d_sb 数值一致 |
| E2 FiLM / E3 attention / E4 MoE | **仍禁止**                        | `e2_allowed=false`；不得用复杂度绕过 E1/E1r 信息结论       |

硬边界未变：B7 仍是默认 RawDSP 头；输出 `raw3`；identifiability v1 的 `information_source_upgrade_required` 不变；不得宣称突破声学 O₂ 上限（仍逼近 B1 ≈ 0.4）。

当前实验边界已调整：实际测量暂时无法进行，项目继续以仿真实验为主。当前 P0 不是继续扩展动态门控，而是在显式 `flow=0` 的静止空气仿真中登记 T/RH/P、L、固定延迟、trigger jitter、SNR 与设备 profile 的参数依据，完成可辨识性、误差预算和独立参数 holdout。仿真通过只能说明所登记数字孪生范围内可行，不能写成真实静止空气或现场能力。

### 证据链要点

- P0 已完成：独立实施计划冻结 E0–E5、输入形状、可部署输入边界、B1 parity 门和停止条件，未改写 B7、D2b 或 identifiability v1。
- P1/E1 首批代码与 clean `tv3-formal-6000` 正式训练已完成；verdict=`frame_fidelity_failed`，`e2_allowed=false`。
- E1r 正式 frame MAE ≈ `0.037` sample（过门），但序列 Ridge 相对 B1 失败 → `b1_parity_failed`。
- E1d / E1d-SB / attachment 正式过门后，问题从“峰位丢失”收敛为“learned sequence representation 相对 RawDSP compact 栈信息不足”。
- E2s-LS 正式过门但不晋升：闭式声速标量几乎不增加 O₂；默认序列表示保持 `e1d_sb_cal_plus_corr_psr_snr_v1`（含 SNR，无 LS）。
- D1 正式：`outputs/tv3_ec_msw/e1d_sb_deploy_probe_s20260704/` → `deploy_probe_passed`。waveform↔cache 四 split 对齐（含 train，`max_abs_diff=0`）；O₂ R² ≈ `0.393 / 0.453 / 0.369`，与 e1d_sb **数值一致**。可选 D2 artifact 打包；**不替换 B7**。

### 相对原文献框架的进展判断

1. **前端位置敏感主张（部分证实）**：E1 失败与 E1r frame 过门，共同支持“过早池化会丢 TOF；可部署峰位坐标必须在下采样前保留”。但这只解决帧级保真，不自动解决组分非劣。
2. **端到端 learned encoder → 组分（未证实）**：E1 / E1r 神经头与冻结 embedding Ridge 均未达到 B1 parity；当前不得进入 FiLM / attention / MoE。
3. **混合路径（已落地旁路）**：用冻结 RawDSP compact 表示（e1d_sb）+ Ridge 恢复 B1 非劣，并用 D1 证明 waveform 部署接线与 cache 一致。这更接近本文摘要中的“原始波形学习分支 + 冻结 RawDSP 审计锚点”的**审计锚点侧**，而非完整 EC-MSW-GatedNet。
4. **动态门控 / FiLM / MoE（未启动）**：文献模块仍可作为远期选项，但 tv3 正式证据要求在 E1 信息充分性之前停止；现有停止条件已触发。
5. **物理信息上限（未突破）**：D1 通过不提高 O₂；LS 几乎无增量。框架收益若存在，应体现在 nuisance 稳健与可部署接线，而非突破单向 TOF 秩亏。
6. **当前分流（仿真优先）**：E2 继续停止；先用静止空气扰动仿真确定是否存在可由模型处理的明确残余机制。只有冻结 B1/B7 在独立 T/RH/P、L、jitter、SNR 或 device-profile holdout 上暴露可学习且信息量充分的残差，才允许另立动态门控启动计划。

实施细节见：

- [archive/completed/tv3_ec_msw_gatednet_implementation_plan.md](../archive/completed/tv3_ec_msw_gatednet_implementation_plan.md)
- [archive/completed/tv3_ec_msw_structured_sequence_head_plan.md](../archive/completed/tv3_ec_msw_structured_sequence_head_plan.md)
- [archive/completed/tv3_ec_msw_e1d_sb_deployable_joint_system_plan.md](../archive/completed/tv3_ec_msw_e1d_sb_deployable_joint_system_plan.md)
- [archive/parked/tv3_static_air_feasibility_implementation_plan.md](../archive/parked/tv3_static_air_feasibility_implementation_plan.md)

### 正式 E1 训练与审计结果（历史证据，保留）
...
冻结 Ridge 能改善 CO₂，却不能恢复 O₂，说明失败不只来自神经输出头。B1 仍能从 RawDSP 获得 `0.37–0.48` 的 O₂ R²，因此当时证据指向 learned encoder 丢失了原始波形中已有的峰位 / TOF 信息，而不是训练未收敛。
...
失败 E1 在 `stride=2`、`avg_pool factor=4` 后仅保留分支激活的 mean/max/centroid，再用 `last/mean/max` 聚合跨帧信息。E1r 已在下采样前补入可部署位置坐标并通过正式 frame fidelity，但 B1 parity 仍失败，说明坐标准确并不保证 sequence representation 充分。后续正式路径已转为 E1d 表示诊断 → E1d-SB / attachment → D1 部署接线；不得通过放宽门限或直接进入 E2 绕过验收。

E1r 已将“冻结坐标 + learned embedding”落实：坐标模板沿用 D2b 的 train-only baseline median 产物，不使用 `exact_simulator_debug` 模板；CLI 在训练开始前读取模板、校验 SHA-256，并把模板值与 digest 写入自包含的 `run_config.model_config`。旧 E1 未配置模板时继续走原路径，正式失败 checkpoint 与配置保持可加载。

正式神经头的 `component_metrics` 当前只落盘 MAE、RMSE 与 R²，缺少预注册要求的 bias；该报告完整性缺口不影响本次失败判定，但后续正式运行应补齐。

### 后续正式对照（E1d-SB / D1）

| split         | B1 O₂ R²（参考） | E1d-SB / D1 O₂ R² | Δ vs B1 |
| ------------- | ------------:| -----------------:| -------:|
| val           | ≈0.428       | 0.393             | ≈−0.035 |
| test          | ≈0.479       | 0.453             | ≈−0.026 |
| extrapolation | ≈0.369       | 0.369             | ≈−0.001 |

D1 与 E1d-SB 指标一致，说明可部署 waveform 路径未引入额外误差，也未带来额外增益。
...
**进展注记（2026-07-17）**：完整 EC-MSW-GatedNet（FiLM / attention / MoE）因 E1/E1r 停止条件尚未启动；已落地的是 e1d_sb compact 旁路与 D1 部署探针（`deploy_probe_passed`），B7 仍为默认头。
...
| 实验           | 变化                                | 主要问题                  | 正式进展（2026-07-17）                  |
| ------------ | --------------------------------- | --------------------- | --------------------------------- |
| E0           | 冻结 B1/B7                          | 当前可信基线                | ✅ 已完成                             |
| E1           | 多尺度位置敏感 encoder + 固定池化            | raw waveform 是否恢复测量信息 | ❌ `frame_fidelity_failed`         |
| E1r          | 冻结峰位坐标 + embedding                | 坐标准确后序列是否够用           | frame ✅ / B1 ❌ `b1_parity_failed` |
| E1d / E1d-SB | RawDSP compact 表示诊断与 builder      | 可部署最小特征集是否非劣 B1       | ✅ `parity_passed`                 |
| attachment   | E1r 帧 + e1d_sb 序列联合               | 联合审计是否成立              | ✅ `attachment_passed`             |
| E2s-LS       | additive SNR 加权闭式 LS              | 是否值得晋升序列表示            | ✅ 过门但不晋升（Δ≈0）                     |
| D1           | waveform→e1d_sb→Ridge 推理探针        | 部署接线是否与 cache 一致      | ✅ `deploy_probe_passed`           |
| E2           | E1 + 环境 FiLM                      | 条件调制是否降低 T/RH/L 域差    | ⛔ `e2_allowed=false`，未启动          |
| E3           | E2 + attentive statistics pooling | 动态窗权重是否优于固定聚合         | ⛔ 未启动                             |
| E4           | E3 + 晚期软 MoE                      | 样本级路由是否必要             | ⛔ 未启动                             |
| E5           | 混合分支与纯端到端对照                       | RawDSP 锚点是否仍必要        | ⏸ D1 仅证明旁路接线；B7 仍默认头              |
...
### P1：位置敏感多尺度前端 — ✅ 代码；❌ 正式 E1；E1r 部分修复

- 只实现 shared multi-scale encoder；
- 用 frame fidelity、峰值位置和固定 Ridge/小 MLP 头验证；
- 不加入 FiLM、MoE 或自监督。
- 正式结论：E1 frame 失败 → E1r frame 过门但 B1 失败 → 转 E1d/E1d-SB/D1 混合旁路。

### P2：环境 FiLM 与注意力统计池化 — ⛔ 停止

- 先加 FiLM，再加 attentive pooling，每次只改变一个因素；
- 使用环境、声程和会话 holdout；
- 保存 gate/attention 诊断。
- **现况**：E1/E1r 未满足进入条件；`e2_allowed=false`。
...
### P4：纯端到端与信息源决策 — 部分旁路已落地

- learned branch 稳定后再移除 RawDSP 锚点（尚未成立）；
- D1 已证明 e1d_sb 旁路可从 waveform 接线，但仍逼近 B1，不替换 B7；
- 若窄窗口仍无绝对误差改善，不扩大网络；
- 将结果作为“数据/模型误差”与“物理信息不足”的分流证据（当前更倾向后者 + 表示不足，而非未收敛）。
...
**2026-07-17 正式证据后的修订判断：**

1. 第一点已部分兑现：E1r 证明可部署峰位坐标可恢复帧保真；但第二、三点（FiLM / gate）尚未获得启动资格。
2. learned encoder 端到端组分路径在当前架构下未通过 B1 parity；继续堆 FiLM/attention/MoE 违反停止条件。
3. 实际前进发生在“混合审计锚点”侧：E1d-SB + D1 把 waveform→compact RawDSP→Ridge→raw3 接到可审计部署探针，指标与 e1d_sb 一致，仍不替换 B7。
4. LS 消融说明额外闭式声速标量几乎无 O₂ 增量；默认表示保持含 SNR 的 e1d_sb。
5. 因此，对该文档原设想的完整 EC-MSW-GatedNet，当前状态是：**文献框架仍可作为远期对照，tv3 主证据线已切到结构化 compact 旁路 + B7 默认头**；窄窗口 / 信息源升级问题仍独立于本门控网络主张。

对 tv3 的最稳妥策略仍是以 B1/B7 为冻结参照。即使旁路接线成功，只要固定窄窗口 P90没有改善，就不能声称精细 O₂检测能力提高。

在实际实验暂不可用的阶段，下一项正式工作是静止空气仿真可辨识性，而不是继续实现完整门控网络：显式固定 `flow=0`，联合扫描测量链 nuisance，并以独立参数 holdout 判断 B1/B7 的残余机制。只有该仿真证据同时满足信息量与业务门，且明确指出固定统计聚合遗漏了什么，动态门控才重新获得启动依据。
