...
>
> **状态更新（2026-07-25）**：`target_p90_o2_error_percent=0.4` 经业务决策**暂缓强制门属性，降级为参考标注**（决策登记见 `../active/tv3_mrs6_hardware_requirements.md` §0）。本文档的推导链与数值保留不变，作为 0.4 参考线的证据来源；已在强制门下判定的历史 verdict（v1 / F4 / F5-wide / MRS-2）不改写。恢复强制门需业务侧再决策并另行登记。
