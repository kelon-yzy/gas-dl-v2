...
| 变量                 | 建议范围              | 用途                       |
| ------------------ | -----------------:| ------------------------ |
| `x_CO2`（narrow）    | 0.03-5.00 %       | 覆盖新鲜空气、通风不足与局部积聚         |
| `x_O2`（narrow）     | 18.00-21.20 %     | 覆盖正常空气到低氧扰动              |
| `x_N2`（narrow）     | 73.80-81.97 %     | 空气主背景；作为直接 label 输出      |
| `x_CO2`（wide，F 线） | 0.03-10.00 %      | 危害监测：超 IDLH 严重积聚包络       |
| `x_O2`（wide，F 线）  | 15.00-25.00 %     | 危害监测：缺氧/富氧包络（OSHA 1910.146） |
| `x_N2`（wide，F 线）  | 65.00-84.97 %     | 残差推导；写入 labels            |
