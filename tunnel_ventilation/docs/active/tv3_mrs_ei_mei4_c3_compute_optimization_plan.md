...
| 阶段 | 状态 | 当前证据 |
| --- | --- | --- |
| P0 | 部分完成 | 已完成缩小契约与双进程微型验证；2026-08-15 补测了单任务成本基线（见 §2.3）；尚未执行 `1/4/8/12/16` worker 性能矩阵。 |
...
| P5 | 未执行 | 未启动正式 C3，未生成 C3 完成 freeze，未更新实验阶段状态。启动前另需处置 [MEI-4 执行计划](tv3_mrs_ei_mei4_execution_plan.md) §0.2 的三项契约审查发现。 |
...
### 2.3 单任务成本实测（2026-08-15，非正式工程基准）

在本机对 B4 已冻结观测做参数空间复解计时。未执行观测空间抽样，未写入 attempt 分片或 freeze。
...
| 子阶段 | 规模 | 单核推算 | 12 worker 理想线性 |
| --- | --- | ---: | ---: |
| M2b | `1296 × 200 × 3 = 777,600` 次求解 | `44.2 h` | `≈ 3.7 h` |
| SBC | 约 `12,000` 次求解 | `≈ 0.7 h` | `≈ 3.4 min` |
| PPC | 约 `248,832` 次前向预测 | `≈ 18 s` 前向部分 | 可忽略 |

三点判读：

1. M2b 占全部成本的九成以上，因此 §6.2 把 M2b 定为每混合物一个分片的细粒度恢复单位是正确的。
2. 2026-07-30 中止时"M2b 需数十小时"的判断成立于当时的串行实现。P1/P2 完成后，全量 C3 的规模已降到单次夜间运行可完成的范围，抽样规模本身不需要缩减。
3. 本表是 P0 基线参考，**不是** P3 的正式基准。正式 worker 数仍须按 §10 P3 对固定基准任务重复至少 3 次后决定；§4.2 的 6–10 倍加速目标以新架构 `workers=1` 为基线，与本表的单核推算不是同一个量。

另需注意：§10 P5 的启动前置不只是本计划的性能门。按 [MEI-4 执行计划](tv3_mrs_ei_mei4_execution_plan.md) §0.2，M2b 在现冻结契约下无法通过完整校准门，T4 触发实现存在循环依赖，组分先验规格在确定性方法与 CC-SBI 之间不一致。这三项属于科学契约层，本计划不处理，也不得由本计划的性能改进绕过。
