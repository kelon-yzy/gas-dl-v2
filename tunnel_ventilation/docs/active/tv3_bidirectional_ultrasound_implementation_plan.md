...
| 2026-07-22 | wide 域立项           | `scope_confirmed_a1_flow_only_b1_parallel`           | 组分宽域 CO₂ 0.03–10%/O₂ 15–25% 独立注册域；A1 仅 F 线 + B1 并行留档已确认；改动契约 `docs/archive/completed/tv3_composition_range_widening_plan.md`；窄域 F0–F5 冻结不动                                                                                                                                       |
...
| 2026-07-22 | F5-S code          | `f5s_code_ready_awaiting_formal_matrix`（历史状态）        | `bidir_spxy_observed_ab_v1`（50 维 AB-only）；`recompute_tv3_split` bidir adapter；`tv3/ml/bidir_f5_secondary.py`；协议 `derive_secondary_selectors=true` + 12 格判据 (d)；窄/宽 `configs/tv3_bidir_model_protocol*.json`；次日宽域正式矩阵已完成，见下行；窄域仍待运行                                               |
...
| 2026-07-23 | F5-wide 正式矩阵       | `f5_model_protocol_failed`                           | `outputs/tv3_bidir/model_protocol_wide/`；6000 条守恒，五臂×双头主矩阵 10 格、F5-S 60 格完整，provenance 无缺项；B1 判据 (a) `0.1165<0.5` 失败，(b)/(c) 通过，(d) 12/12 通过；pair 声速偏差 `1.8067→0.03681 m/s`，但判据 (e) 误用全时序 frame-vs-sequence P95 `16.60 μs` 对比 F3 steady 多 L 门 `0.10 μs`，记为审计口径问题，不改写判据 (a) 的正式失败 |
...
F5 要点（宽域正式矩阵已完成并失败；窄域正式矩阵仍待服务器）：
...
- 宽域正式结果：B1 A1/A3 的 S-Flow OOD O₂ MAE=`1.23635/1.11981`，改善 `0.11654 vol%`，未达 `0.5`；B7 对应 `0.81749/0.70392`，方向一致但不是主门。A3 与 A2 差距、零流锚点及 12 格 selector 门均通过，A4 无额外收益，A5 接近 A3，说明主要收益来自 BA 与解耦声速块而非 v̂ 单列。
- 判据 (e) 审计缺口：F3 门使用 steady accepted 帧的多 L mid-pair TOF 拟合截距，F5 `reciprocity_residual_p95_s` 使用所有 phase 的 frame ĉ 相对整段 `c_seq`；正式集还含 `3 μs` 独立 jitter。两者统计对象不同，后续只能统一统计量后重算审计，不得调低冻结阈值或重训模型规避判据 (a)。
- F5-S：B1 12 格 ΔR²范围 `0.02422–0.06561`，全部高于 `−0.01`；S-Y 三个 seed 的 OOD hash 相同，三格不是独立 OOD 重复，S-L 三个 OOD hash 不同。
