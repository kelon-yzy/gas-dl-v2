...
> 依据：[掘进通风项目记忆库.md](../legacy/掘进通风项目记忆库.md)、[三组分检测深度学习新框架方案.md](../legacy/三组分检测深度学习新框架方案.md)、现有 `tv3.dl` 训练入口与 D0 clean 6000 结果。
...
- **后续**：可部署 baseline 回到 D0-observed；R5' TabPFN 判据通过（val O₂ 0.6673）；R5 小 MLP 判据未通过（val O₂ −0.183）。下一步 P0 = O₂ 光学通道（TDLAS）。详见记忆库 §6.4 末、§6.8、§6.9。
