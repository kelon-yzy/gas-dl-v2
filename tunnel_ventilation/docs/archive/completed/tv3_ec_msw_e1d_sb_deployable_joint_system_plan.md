...
> 状态（2026-07-17）：**D1 正式 `deploy_probe_passed`。**  
...
### D1 — 推理探针（正式 `deploy_probe_passed`）
...
- [x] D0：本文冻结契约与门禁
- [x] D1 代码：inference / deploy_probe / CLI / configs / tests
- [x] D1 正式：`outputs/tv3_ec_msw/e1d_sb_deploy_probe_s20260704/` → `deploy_probe_passed`
  - waveform↔cache：train/val/test/extrapolation `max_abs_diff=0`（atol=`1e-5`）
  - O₂ R²：val/test/extrapolation ≈ `0.393 / 0.453 / 0.369`（与 e1d_sb 数值一致；相对 B1 ΔO₂ 在门内）
  - 身份门：`e1d_sb_identity_ok` / `attachment_identity_ok`；`ls_promoted=false`；`default_head_remains=B7`；`e2_allowed=false`
- [ ] D2：可选 artifact 打包（仅正式 `deploy_probe_passed` 后；仍不替换 B7）
