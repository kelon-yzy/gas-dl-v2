...
per-timestep z-score 会去掉绝对幅度/能量,而声学衰减和能量仍可能对 CO₂/O₂/N₂ 有贡献。当前 v3 系列启用:

```json
"waveform_stats_features": "log_std,log_max_abs"
```

数据层在归一化前为每个波形模态提取逐帧 `log_std` 与 `log_max_abs`,并按 `slow + waveform_stats + waveform` 顺序输入。对 `slow,ultrasonic` 配置,CLI 会把 fusion 模型 slow 分支从 7 维自动扩展到 9 维。
...
- `normalize_waveforms=true` 必须同时满足 `dequantize_waveforms=true` 与 `waveform_adc_scale=1.0`。
- CLI 支持 `--no-normalize-waveforms` / `--no-dequantize-waveforms` 做显式回退。
- `resolved_loss.component_weights` 会写入 `metrics.json` 与 `run_config.json`。
- `film_gate` 必须叠加 `fusion_layer_norm=true`,GatedFusion 中性初始化为初始等价 concat。
