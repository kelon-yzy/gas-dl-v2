...
<!-- file-state: revision=34 | updated-at=2026-06-02T16:19:03+08:00 | writer-id=Claude | base-workspace-revision=65 -->
...
- 时序实验阶段划分现状（已核实）：单条序列 128 步/64s，src/sim/generation/phases.py 固定四分位切 baseline/exposure/steady/recovery 各 32 步；CNN1D/TCN 末端均 AdaptiveAvgPool1d(1) 全局平均池化、TCN 默认感受野仅 29 步（dilations=(1,2,4)）——当前设定下长时序 DL 优势无法体现，已产出改进提案 docs/LONG_SEQUENCE_PROTOCOL_PROPOSAL_2026-06-02.md 待评审。
...
- 时序协议改进遵循配置驱动+向后兼容+可证伪：默认保持现状可复现（short 档位逐位重现 wv4-smoke），长时序优势以『序列感知模型 vs 顺序不敏感强 baseline 的 R² 差距随 T 增大而扩大』验收（回应 DLinear 警示）。
...
- 当前时序设定（128 步固定四分位 + 全局平均池化 + TCN RF=29）使长时序 DL 优势无法体现；改进须 P2 事件定位 + P4 大感受野/保序聚合配套，单纯拉长无效。
- context_brief 两处与代码脱节待订正：registry 实际仅注册 cnn1d/tcn（LSTM/Transformer 未实现）；Phase 2 计划项（声程配置化/物理单测/LHS 收尾）实际未动工。

<!-- section: next_step -->
- 评审长时序协议改进提案（docs/LONG_SEQUENCE_PROTOCOL_PROPOSAL_2026-06-02.md）；若采纳先做 S0+S1+S5 最小闭环（PhaseSchedule 抽象+兼容层 / 拉长+TCN 感受野随 T 缩放 / 强 baseline 对比），验证长时序优势是否随 T 拉开。
...
- 2026-06-02：完成时序阶段划分分析 + 联网调研，产出长时序协议改进提案（docs/LONG_SEQUENCE_PROTOCOL_PROPOSAL_2026-06-02.md，16 篇文献）；诊断长时序 DL 优势不显的三根因（短序列/阶段固定/全局池化+TCN RF=29）。
