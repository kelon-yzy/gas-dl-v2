...
- 声学模型契约：`manifest.json` 和 `metadata/waveform_spec.json` 会记录 `ultrasonic_model = "simplified_tof_proxy_v1"`、`fiber_mic_model = "acoustic_proxy_v1"`、`fiber_optical_demodulation_model = "not_implemented"`、`acoustic_attenuation_model = "semi_empirical_relaxation_proxy_v1"`。当前 `fiber_mic` 是光纤声学传感链路的简化代理模型，不声明为完整光纤干涉解调链路。
...
- 验证基线：新机器 Python 3.12.10 虚拟环境已安装 `numpy/scipy/torch/pytest/hitran-api` 核心依赖，`python -m pytest tests` 当前为 143 passed。
...
- 独立 ML feature package 落盘导出尚未迁移；传统 ML baseline 的内存特征矩阵、Mean/Ridge 训练和 split 评估入口已落地。
