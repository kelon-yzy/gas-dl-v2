...
- HITRAN 光谱积分支撑：`src/sim/generation/spectral/` 已具备表格谱积分、HAPI 适配、缓存、单位换算和默认滤光片/网格配置；`src/pipeline/precompute_hitran_spectra.py` 已在本地真实 HAPI 环境下载 CH4/CO2/H2O 两个 NDIR 窗口谱线，缓存目录为本地运行产物。
...
- 更复杂的环境串扰参数化。
- 目标传感器滤光片响应配置、PNNL/NIST 外部谱表对照和谱源版本管理；当前已具备本地表格谱积分原型、HAPI 适配层、真实 HITRAN 下载验证和谱线缓存。
