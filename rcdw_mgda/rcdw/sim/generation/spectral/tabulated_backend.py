"""tabulated 后端：基于预生成谱线表计算 NDIR 通道吸光度。

完全等价 HG 主线 ``src/sim/generation/spectral/tabulated_backend.py``。
"""
...
def prepare_tabulated_spectra(
    *,
    ...
    for spectrum in spectra:
        if spectrum.wavenumber_cm1.shape != wavenumber.shape or not np.allclose(
            spectrum.wavenumber_cm1, wavenumber
        ):
        ...
        if spectrum.absorption_coeff_per_percent_m.shape != wavenumber.shape:
            raise ValueError(
                "absorption_coeff_per_percent_m must match wavenumber_cm1 shape"
            )
...
def compute_prepared_tabulated_ndir_absorbance(
    *,
    ...
    absorbance_by_gas: dict[str, float] = {}
    ...
    for spectrum in prepared.spectra:
        ...
        gas_optical_depth = (
            spectrum.absorption_coeff_per_percent_m.astype(np.float64)
            * concentration
            * path_length_m
        )
