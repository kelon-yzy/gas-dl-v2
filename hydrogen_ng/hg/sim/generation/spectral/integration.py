...
def integrate_channel_absorbance(
    *,
    ...
    channel_transmittance = float(np.trapezoid(filter_response * transmittance, wavenumber_cm1) / filter_area)
...
def _validate_same_shape(*arrays: np.ndarray) -> None:
    if len({array.shape for array in arrays}) != 1:
        raise ValueError("wavenumber_cm1, optical_depth, and filter_response must have the same shape")
    if arrays[0].ndim != 1:
